...
(function(window, undefined) {
			...
			if (typeof selector === "string") {
				...
				if (match && (match[1] || !context)) {
					...
					if (match[1]) {
						...
						doc = (context && context.nodeType ? context.ownerDocument || context :
							document);
	...
	jQuery.extend({
		...
		parseHTML: function(data, context, scripts) {
			...
			return jQuery.merge([], (parsed.cacheable ? jQuery.clone(parsed.fragment) :
				...
				if (typeof data === "string") {
					try {
						data = data === "true" ? true : data === "false" ? false : data ===
							...
							+data + "" === data ? +data : rbrace.test(data) ? jQuery.parseJSON(data) :
							data;
					} catch (e) {}
		...
		toggleClass: function(value, stateVal) {
			...
			return this.each(function() {
				if (type === "string") {
					...
					this.className = this.className || value === false ? "" : jQuery._data(
						this, "__className__") || "";
				}
			});
		},
		...
		attrHooks: {
			type: {
				set: function(elem, value) {
					...
					} else if (!jQuery.support.radioValue && value === "radio" && jQuery.nodeName(
						elem, "input")) {
	...
	boolHook = {
		get: function(elem, name) {
			...
			return property === true || typeof property !== "boolean" && (attrNode =
					elem.getAttributeNode(name)) && attrNode.nodeValue !== false ? name.toLowerCase() :
				undefined;
		},
		set: function(elem, value, name) {
			var propName;
	...
	jQuery.event = {
			...
			for (t = 0; t < types.length; t++) {
				...
				for (j = 0; j < eventType.length; j++) {
					...
					if ((mappedTypes || origType === handleObj.origType) && (!handler ||
						handler.guid === handleObj.guid) && (!namespaces || namespaces.test(
						handleObj.namespace)) && (!selector || selector === handleObj.selector ||
		...
		trigger: function(event, data, elem, onlyHandlers) {
			...
			if ((!elem || jQuery.event.customEvent[type]) && !jQuery.event.global[
				type]) {
			...
			if (!onlyHandlers && !event.isDefaultPrevented()) {
				if ((!special._default || special._default.apply(elem.ownerDocument,
						data) === false) && !(type === "click" && jQuery.nodeName(elem, "a")) &&
					...
					if (cur.disabled !== true || event.type !== "click") {
						...
						for (i = 0; i < delegateCount; i++) {
							...
							if (selMatch[sel] === undefined) {
								selMatch[sel] = handleObj.needsContext ? jQuery(sel, this).index(cur) >=
		...
		mouseHooks: {
			...
			filter: function(event, original) {
				...
				if (event.pageX == null && original.clientX != null) {
					...
					body = eventDoc.body;
					event.pageX = original.clientX + (doc && doc.scrollLeft || body && body
						.scrollLeft || 0) - (doc && doc.clientLeft || body && body.clientLeft ||
						0);
	...
	jQuery.Event = function(src, props) {
		...
		if (src && src.type) {
			...
			this.isDefaultPrevented = (src.defaultPrevented || src.returnValue ===
					false || src.getPreventDefault && src.getPreventDefault()) ? returnTrue :
				returnFalse;
			// Event type
		} else {
			this.type = src;
		...
		off: function(types, selector, fn) {
			...
			if (types && types.preventDefault && types.handleObj) {
				...
				jQuery(types.delegateTarget).off(handleObj.namespace ? handleObj.origType +
					"." + handleObj.namespace : handleObj.origType, handleObj.selector,
					handleObj.handler);
				return this;
			}
			if (typeof types === "object") {
	...
	jQuery.each((
		...
		function(i, name) {
			...
			jQuery.fn[name] = function(data, fn) {
				...
				return arguments.length > 0 ? this.on(name, null, data, fn) : this.trigger(
					name);
			};
		...
		var cachedruns,
			...
			assertHrefNotNormalized = assert(function(div) {
				...
				return div.firstChild && typeof div.firstChild.getAttribute !==
					strundefined && div.firstChild.getAttribute("href") === "#";
			}),
					...
					if ((m = match[1])) {
						if (nodeType === 9) {
							...
							if (context.ownerDocument && (elem = context.ownerDocument.getElementById(
								m)) && contains(context, elem) && elem.id === m) {
								results.push(elem);
								return results;
							}
						}
			...
			*/
					...
					return function(elem, context) {
						...
						return operator === "=" ? result === check : operator === "!=" ?
							result !== check : operator === "^=" ? check && result.indexOf(
								check) === 0 : operator === "*=" ? check && result.indexOf(check) >
							-1 : operator === "$=" ? check && result.substr(result.length -
			...
			pseudos: {
				...
				"text": function(elem) {
					...
					return elem.nodeName.toLowerCase() === "input" && (type = elem.type) ===
						"text" && ((attr = elem.getAttribute("type")) == null || attr.toLowerCase() ===
							type);
				},
				// Input types
				...
				"gt": createPositionalPseudo(function(matchIndexes, length, argument) {
					for (var i = argument < 0 ? argument + length : argument; ++i <
						length;) {
						matchIndexes.push(i);
					}
					return matchIndexes;
			...
			return markFunction(function(seed, results, context, xml) {
				...
				if (seed) {
					if (postFinder || preFilter) {
						...
						i = matcherOut.length;
						while (i--) {
							if ((elem = matcherOut[i]) && (temp = postFinder ? indexOf.call(
					...
					matcherOut = condense(matcherOut === results ? matcherOut.splice(
			...
			for (; i < len; i++) {
				if ((matcher = Expr.relative[tokens[i].type])) {
					...
					if (matcher[expando]) {
						...
						return setMatcher(i > 1 && elementMatcher(matchers), i > 1 && tokens.slice(
								0, i - 1).join("").replace(rtrim, "$1"), matcher, i < j &&
							matcherFromTokens(tokens.slice(i, j)), j < len && matcherFromTokens(
								(tokens = tokens.slice(j))), j < len && tokens.join(""));
					}
		...
		function select(selector, context, results, seed, xml) {
			...
			if (!seed) {
				...
				if (match.length === 1) {
					...
					if (tokens.length > 2 && (token = tokens[0]).type === "ID" && context.nodeType ===
						9 && !xml && Expr.relative[tokens[1].type]) {
					...
					for (i = matchExpr["POS"].test(selector) ? -1 : tokens.length - 1; i >=
						...
						if ((find = Expr.find[type])) {
							...
							if ((seed = find(token.matches[0].replace(rbackslash, ""), rsibling.test(
								tokens[0].type) && context.parentNode || context, xml))) {
			...
			compile(selector, match)(seed, context, xml, results, rsibling.test(
				selector));
			return results;
		}
	...
	jQuery.fn.extend({
		...
		is: function(selector) {
			return !!selector && (typeof selector === "string" ?
				...
				rneedsContext.test(selector) ? jQuery(selector, this.context).index(
					this[0]) >= 0 : jQuery.filter(selector, this).length > 0 : this.filter(
					selector).length > 0);
		},
		...
		addBack: function(selector) {
			return this.add(selector == null ? this.prevObject : this.prevObject.filter(
				selector));
		}
	});
	...
	jQuery.each({
		...
		contents: function(elem) {
			return jQuery.nodeName(elem, "iframe") ? elem.contentDocument || elem.contentWindow
				.document : jQuery.merge([], elem.childNodes);
		}
	}, function(name, fn) {
	...
	jQuery.fn.extend({
		...
		html: function(value) {
			return jQuery.access(this, function(value) {
				...
				if (typeof value === "string" && !rnoInnerhtml.test(value) && (jQuery
					.support.htmlSerialize || !rnoshimcache.test(value)) && (jQuery.support
		...
		replaceWith: function(value) {
			...
			return this.length ? this.pushStack(jQuery(jQuery.isFunction(value) ?
				value() : value), "replaceWith", value) : this;
		},
				...
				if (first) {
					...
					for (iNoClone = results.cacheable || l - 1; i < l; i++) {
						callback.call(table && jQuery.nodeName(this[i], "table") ?
							findOrAppend(this[i], "tbody") : this[i], i === iNoClone ? fragment :
	...
	jQuery.buildFragment = function(args, context, scripts) {
		...
		if (args.length === 1 && typeof first === "string" && first.length < 512 &&
			...
			(jQuery.support.checkClone || !rchecked.test(first)) && (jQuery.support.html5Clone ||
	...
	jQuery.extend({
		clone: function(elem, dataAndEvents, deepDataAndEvents) {
			var srcElements,
				...
				i,
				clone;
			if (jQuery.support.html5Clone || jQuery.isXMLDoc(elem) || !rnoshimcache.test(
				"<" + elem.nodeName + ">")) {
				...
				if (typeof elem === "string") {
					if (!rhtml.test(elem)) {
						...
						if (!jQuery.support.tbody) {
							...
							tbody = tag === "table" && !hasBody ? div.firstChild && div.firstChild
								.childNodes :
	...
	(function() {
		...
		jQuery.uaMatch = function(ua) {
			ua = ua.toLowerCase();
			var match = /(chrome)[ \/]([\w.]+)/.exec(ua) || /(webkit)[ \/]([\w.]+)/.exec(
					ua) || /(opera)(?:.*version|)[ \/]([\w.]+)/.exec(ua) ||
		...
		for (; index < length; index++) {
			...
			if (show) {
				...
				if (elem.style.display === "" && isHidden(elem)) {
					values[index] = jQuery._data(elem, "olddisplay", css_defaultDisplay(elem
						.nodeName));
				}
			} else {
	...
	jQuery.fn.extend({
		css: function(name, value) {
			return jQuery.access(this, function(elem, name, value) {
				return value !== undefined ? jQuery.style(elem, name, value) : jQuery
					.css(elem, name);
	...
	function getWidthOrHeight(elem, name, extra) {
			...
			return (val + augmentWidthOrHeight(elem, name, extra || (isBorderBox ?
				"border" : "content"), valueIsBorderBox)) + "px";
		}
	...
	function css_defaultDisplay(nodeName) {
		...
		if (display === "none" || display === "") {
			...
			iframe = document.body.appendChild(iframe || jQuery.extend(document.createElement(
				"iframe"), {
				frameBorder: 0,
				width: 0,
				height: 0
		...
		jQuery.cssHooks[name] = {
			...
			set: function(elem, value, extra) {
				return setPositiveNumber(elem, value, extra ? augmentWidthOrHeight(
					elem, name, extra, jQuery.support.boxSizing && jQuery.css(elem,
		...
		jQuery.cssHooks.opacity = {
			get: function(elem, computed) {
				...
				return ropacity.test((computed && elem.currentStyle ? elem.currentStyle.filter :
						elem.style.filter) || "") ? (0.01 * parseFloat(RegExp.$1)) + "" :
					computed ? "1" : "";
			},
			set: function(elem, value) {
	...
	if (jQuery.expr && jQuery.expr.filters) {
		jQuery.expr.filters.hidden = function(elem) {
			return (elem.offsetWidth === 0 && elem.offsetHeight === 0) || (!jQuery.support
				.reliableHiddenOffsets && ((elem.style && elem.style.display) || curCSS(
					elem, "display")) === "none");
		};
	...
	jQuery.extend({
		...
		*/
		...
		ajax: function(url, options) {
			...
			jqXHR.setRequestHeader("Accept", s.dataTypes[0] && s.accepts[s.dataTypes[
				0]] ? s.accepts[s.dataTypes[0]] + (s.dataTypes[0] !== "*" ? ", " +
	...
	function Animation(elem, properties, options) {
		var result,
			...
			animation = deferred.promise({
				...
				createTween: function(prop, end, easing) {
					var tween = jQuery.Tween(elem, animation.opts, prop, end, animation.opts
						.specialEasing[prop] || animation.opts.easing);
		...
		for (; index < length; index++) {
			result = animationPrefilters[index].call(animation, elem, props, animation
				.opts);
			if (result) {
				return result;
			}
	...
	Tween.prototype = {
		...
		cur: function() {
			...
			return hooks && hooks.get ? hooks.get(this) : Tween.propHooks._default.get(
				this);
		},
	...
	Tween.propHooks = {
		_default: {
			get: function(tween) {
				var result;
				if (tween.elem[tween.prop] != null && (!tween.elem.style || tween.elem.style[
					tween.prop] == null)) {
	...
	jQuery.each(["toggle", "show", "hide"], function(i, name) {
		...
		jQuery.fn[name] = function(speed, easing, callback) {
			return speed == null || typeof speed === "boolean" ||
				...
				(!i && jQuery.isFunction(speed) && jQuery.isFunction(easing)) ? cssFn.apply(
					this, arguments) : this.animate(genFx(name, true), speed, easing,
					callback);
		};
	});
	jQuery.fn.extend({
	...
	jQuery.speed = function(speed, easing, fn) {
		...
		opt.duration = jQuery.fx.off ? 0 : typeof opt.duration === "number" ? opt.duration :
			opt.duration in jQuery.fx.speeds ? jQuery.fx.speeds[opt.duration] : jQuery
			.fx.speeds._default;
	...
	jQuery.each({
		...
		jQuery.fn[method] = function(val) {
			return jQuery.access(this, function(elem, method, val) {
				...
				if (win) {
					win.scrollTo(!top ? val : jQuery(win).scrollLeft(), top ? val :
...
(function($) {
	...
	function Scrollbar(root, options) {
		var oSelf = this,
			oWrapper = root,
			oViewport = {
				obj: $(options.viewportSelector, root)
			},
			oContent = {
				obj: $(options.overviewSelector, root)
			},
			oScrollbar = {
				obj: $(options.scrollbarSelector, root)
			},
			oTrack = {
				obj: $(options.trackSelector, oScrollbar.obj)
			},
			oThumb = {
				obj: $(options.thumbSelector, oScrollbar.obj)
			},
		...
		function initialize() {
			...
			var userSelect = 'userSelect' in style || 'msUserSelect' in style ||
				'MozUserSelect' in style || 'webkitUserSelect' in style;
			if (!userSelect) {
		...
		this.update = function(sScroll) {
			...
			oTrack[options.axis] = options.size === 'auto' ? oViewport[options.axis] :
				options.size;
			oThumb[options.axis] = Math.min(oTrack[options.axis], Math.max(0, (
				options.sizethumb === 'auto' ? (oTrack[options.axis] * oContent.ratio) :
				options.sizethumb)));
			...
			oScrollbar.ratio = options.sizethumb === 'auto' ? (oContent[options.axis] /
					oTrack[options.axis]) : (oContent[options.axis] - oViewport[options.axis]) /
				(oTrack[options.axis] - oThumb[options.axis]);
			iScroll = (sScroll === 'relative' && oContent.ratio <= 1) ? Math.min((
					oContent[options.axis] - oViewport[options.axis]), Math.max(0, iScroll)) :
				0;
			iScroll = (sScroll === 'bottom' && oContent.ratio <= 1) ? (oContent[
					options.axis] - oViewport[options.axis]) : isNaN(parseInt(sScroll, 10)) ?
		...
		function wheel(event) {
			if (oContent.ratio < 1) {
				var oEvent = event || window.event,
					iDelta = oEvent.wheelDelta ? oEvent.wheelDelta / 120 : -oEvent.detail /
					3;
				...
				oThumb.obj.css(sDirection, iScroll / oScrollbar.ratio);
				oContent.obj.css(sDirection, -iScroll);
				if (options.lockscroll || (iScroll !== (oContent[options.axis] -
					oViewport[options.axis]) && iScroll !== 0)) {
			...
			if (oContent.ratio < 1) {
				if (options.invertscroll && touchEvents) {
					iPosition.now = Math.min((oTrack[options.axis] - oThumb[options.axis]),
						Math.max(0, (iPosition.start + (iMouse.start - (sAxis ? event.pageX :
							event.pageY)))));
				} else {
					iPosition.now = Math.min((oTrack[options.axis] - oThumb[options.axis]),
						Math.max(0, (iPosition.start + ((sAxis ? event.pageX : event.pageY) -
							iMouse.start))));
				}
...
jQuery.effects || function(e, t) {
	...
	e.effects = {
		...
		function(t, n) {
			function p(e, t, n) {
				var r = a[t.type] || {};
				return e == null ? n || !t.def ? null : t.def : (e = r.floor ? ~~e :
					parseFloat(e), isNaN(e) ? t.def : r.mod ? (e + r.mod) % r.mod : 0 > e ?
					0 : r.max < e ? r.max : e)
			}
			...
			function d(e) {
				...
				return e = e.toLowerCase(), h(s, function(t, i) {
						var s, o = i.re.exec(e),
							a = o && i.parse(o),
							f = i.space || "rgba";
						if (a) return s = n[f](a), n[u[f].cache] = s[u[f].cache], r = n._rgba =
							s._rgba, !1
					}), r.length ? (r.join() === "0,0,0,0" && t.extend(r, c.transparent), n) :
					c[e]
			}

			function v(e, t, n) {
				return n = (n + 1) % 1, n * 6 < 1 ? e + (t - e) * n * 6 : n * 2 < 1 ? t :
			...
			l.style.cssText = "background-color:rgba(1,1,1,.5)", f.rgba = l.style.backgroundColor
				.indexOf("rgba") > -1, h(u, function(e, t) {
					...
					transition: function(e, t) {
						...
						return n = n[i.cache], h(i.props, function(e, r) {
							var i = r.idx,
								...
								u = a[r.type] || {};
							if (o === null) return;
							s === null ? l[i] = o : (u.mod && (o - s > u.mod / 2 ? s += u.mod :
					...
					toHslaString: function() {
						var e = "hsla(",
							n = t.map(this.hsla(), function(e, t) {
								return e == null && (e = t > 2 ? 1 : 0), t && t < 3 && (e = Math.round(
									e * 100) + "%"), e
							});
					...
					if (e[0] == null || e[1] == null || e[2] == null) return [null, null,
						null, e[3]
					];
					var t = e[0] / 255,
						n = e[1] / 255,
						...
						f = a * .5,
						l, c;
					return o === s ? l = 0 : t === s ? l = 60 * (n - r) / u + 360 : n === s ?
						l = 60 * (r - t) / u + 120 : l = 60 * (t - n) / u + 240, f === 0 || f ===
						1 ? c = f : f <= .5 ? c = u / a : c = u / (2 - a), [Math.round(l) % 360,
							c, f, i == null ? 1 : i
						]
					...
					var t = e[0] / 360,
						...
						o = 2 * r - s,
						u, a, f;
					return [Math.round(v(o, s, t + 1 / 3) * 255), Math.round(v(o, s, t) * 255),
					...
					o.fn[e] = function(e) {
						...
						o.fn[n] = function(s) {
							var o = t.type(s),
								...
								f = a[r.idx],
								l;
							return o === "undefined" ? f : (o === "function" && (s = s.call(
								this, f), o = t.type(s)), s == null && r.empty ? this : (o ===
								"string" && (l = i.exec(s), l && (s = f + parseFloat(l[2]) * (l[
									1] === "+" ? 1 : -1))), a[r.idx] = s, this[u](a)))
						}
					})
				}), h(r, function(e, n) {
					t.cssHooks[n] = {
						...
						e.colorInit || (e.start = o(e.elem, n), e.end = o(e.end), e.colorInit = !
							0), t.cssHooks[n].set(e.elem, e.start.transition(e.end, e.pos))
					}
		...
		function() {
			function i() {
				var t = this.ownerDocument.defaultView ? this.ownerDocument.defaultView.getComputedStyle(
						this, null) : this.currentStyle,
					n = {},
					r, i, s;
				if (t && t.length && t[0] && t[t[0]]) {
					s = t.length;
					while (s--) r = t[s], typeof t[r] == "string" && (n[e.camelCase(r)] = t[
						r])
				} else
			...
			function s(t, n) {
				var i = {},
					s, o;
				for (s in n) o = n[s], t[s] !== o && !r[s] && (e.fx.step[s] || !isNaN(
					parseFloat(o))) && (i[s] = o);
				return i
			}
			...
			e.each(["borderLeftStyle", "borderRightStyle", "borderBottomStyle",
				...
				e.fx.step[n] = function(e) {
					if (e.end !== "none" && !e.setAttr || e.pos === 1 && !e.setAttr) jQuery
						.style(e.elem, n, e.end), e.setAttr = !0
				}
				...
				return this.queue(function() {
					...
					f = f.map(function() {
						...
						return this.end = i.call(this.el[0]), this.diff = s(this.start,
							this.end), this
				...
				toggleClass: function(n, r, i, s, o) {
					return typeof r == "boolean" || r === t ? i ? e.effects.animateClass.call(
						this, r ? {
							add: n
						} : {
							remove: n
						}, i, s, o) : this._toggleClass(n, r) : e.effects.animateClass.call(
						this, {
							toggle: n
						}, r, i, s)
				},
		...
		function() {
			function i(n, r, i, s) {
				...
				return e.isFunction(i) && (s = i, i = null), r && e.extend(n, r), i = i ||
					r.duration, n.duration = e.fx.off ? 0 : typeof i == "number" ? i : i in
					e.fx.speeds ? e.fx.speeds[i] : e.fx.speeds._default, n.complete = s || r
					.complete, n
			}

			function s(t) {
				return !t || typeof t == "number" || e.fx.speeds[t] ? !0 : typeof t ==
					"string" && !e.effects.effect[t] ? n && e.effects[t] ? !1 : !0 : !1
			}
			e.extend(e.effects, {
				version: "1.9.0",
				save: function(e, t) {
					for (var n = 0; n < t.length; n++) t[n] !== null && e.data(r + t[n], e[
						0].style[t[n]])
				},
				restore: function(e, n) {
					var i, s;
					for (s = 0; s < n.length; s++) n[s] !== null && (i = e.data(r + n[s]),
						i === t && (i = ""), e.css(n[s], i))
				},
				...
				removeWrapper: function(t) {
					...
					return t.parent().is(".ui-effects-wrapper") && (t.parent().replaceWith(
						t), (t[0] === n || e.contains(t[0], n)) && e(n).focus()), t
				},
				...
				effect: function(t, r, s, o) {
					function h(t) {
						...
						var n = e(this),
							r = u.complete,
							i = u.mode;
						(n.is(":hidden") ? i === "hide" : i === "show") ? s(): l.call(n[0],
							u, s)
					}
					...
					return e.fx.off || !l && !c ? a ? this[a](u.duration, u.complete) :
						this.each(function() {
							u.complete && u.complete.call(this)
						}) : l ? f === !1 ? this.each(h) : this.queue(f || "fx", h) : c.call(
							this, {
								options: u,
								duration: u.duration,
								callback: u.complete,
				...
				toggle: function(t) {
					if (s(t) || typeof t == "boolean" || e.isFunction(t)) return this.__toggle
		...
		function() {
			...
			e.each(["Quad", "Cubic", "Quart", "Quint", "Expo"], function(e, n) {
				...
				Elastic: function(e) {
					return e === 0 || e === 1 ? e : -Math.pow(2, 8 * (e - 1)) * Math.sin((
				...
				Bounce: function(e) {
					...
					return 1 / Math.pow(4, 3 - n) - 7.5625 * Math.pow((t * 3 - 2) / 22 - e,
						2)
				}
...
(function(e, t) {
	e.effects.effect.slide = function(t, n) {
		var r = e(this),
			...
			f = u === "up" || u === "left",
			l, c = {};
		e.effects.save(r, i), r.show(), l = t.distance || r[a === "top" ?
			"outerHeight" : "outerWidth"](!0), e.effects.createWrapper(r).css({
			overflow: "hidden"
		}), o && r.css(a, f ? isNaN(l) ? "-" + l : -l : l), c[a] = (o ? f ? "+=" :
			"-=" : f ? "-=" : "+=") + l, r.animate(c, {
			queue: !1,
			duration: t.duration,
			easing: t.easing,
			complete: function() {
				s === "hide" && r.hide(), e.effects.restore(r, i), e.effects.removeWrapper(
					r), n()
			}
		})
	}
	...
	*/
	...
	function showExpressInstall(att, par, replaceElemIdStr, callbackFn) {
			...
			if (obj) {
				...
				if (typeof att.height == UNDEF || (!/%$/.test(att.height) && parseInt(att.height,
					10) < 137)) {
					att.height = "137";
				}
