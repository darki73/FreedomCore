...
Class DBCReader {
    ...
    public function __construct($DBCPath, $FileData){
        ...
        $this->FilePath = $DBCPath.$this->FileName;
    ...
    private function getFileHeader(){
        $this->HeaderStructure = unpack('VrecordCount/VfieldCount/VrecordSize/VstringSize', fread($this->FileHandler, 16));
    }

    private function VerifyFileSize(){
        if($this->FileSize < 20)
            die("File " . $this->FileName . " is too small for a DBC file\n");
    }

    private function VerifyMagicBytes(){
        if(fread($this->FileHandler, 4) != $this->MagicBytes)
